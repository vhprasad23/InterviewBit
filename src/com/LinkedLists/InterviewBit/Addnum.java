package com.LinkedLists.InterviewBit;
...
public class Addnum {
    public ListNode addTwoNumbers(ListNode a, ListNode b) {
        ...
        while(aCopy != null && bCopy != null){
            vals.add((aCopy.val + bCopy.val + carry) % 10);
            carry = (aCopy.val + bCopy.val + carry)/10;
            aCopy = aCopy.next;
            bCopy = bCopy.next;
        ...
        while(aCopy !=null){
            vals.add((aCopy.val + carry) % 10);
            carry = (aCopy.val + carry)/10;
            aCopy = aCopy.next;
        }

        while(bCopy !=null){
            vals.add((bCopy.val + carry) % 10);
            carry = (bCopy.val + carry)/10;
            bCopy = bCopy.next;
        }
